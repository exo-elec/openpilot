...
tinygrad/extra/usbgpu/patch.py and runtime/ops_amd.py's USBIface). The
pre-flash ROM-bootloader IDs (0x174C:0x2464, 0x174C:0x2463) are
deliberately not checked here — a ROM-state device needs flashing before
it's usable as a GPU backend, which this stub does not perform.
...
def _detect_usb_gpu() -> bool:
  ...
  for path in glob.glob('/sys/bus/usb/devices/*'):
    try:
      ...
      continue
    if (f'0x{vendor}', f'0x{product}') in POST_FLASH_VID_PIDS:
      return True
  return False
