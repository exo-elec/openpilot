...
02M's mic array geometry (channel count, spacing, orientation) is NOT known
yet -- micd captures one channel today (system/hardware/i2s_audio.py), so the
beamformer runs as a pass-through until the board's array is documented. Do
not fill in MicArray from VisionPilot's defaults (4 ch, 6 cm): those were
