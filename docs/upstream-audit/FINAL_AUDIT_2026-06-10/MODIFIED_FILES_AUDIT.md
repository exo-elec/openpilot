...
| `.gitmodules` | **KEEP** | panda/opendbc/teleoprtc/tinygrad submodules removed (no panda HW; tinygrad unused on RK); RK-port submodules added (mpp, rga, rknpu2, hailort, arm_compute, clblast, valhalla, udsoncan, isotp, pygnssutils, carla[update=none]). msgq → exo-electronics fork (real EOP commits: VISION_STREAM_REAR, ION removal). |
