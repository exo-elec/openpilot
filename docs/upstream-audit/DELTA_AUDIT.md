...
| 2026-09-01 | **Audited `selfdrive/ui/qt` (76-file diff vs `v0.10.0`) — full CONTROLS_AUDIT.md-style review.** Split 24 net-new files (Group A, no audit needed), 4 cleanly-deleted-not-modified files (Group C, verified no dangling references — `driverview.*` removed with its only caller; `driver_monitoring.*`'s DMS overlay relocated into `hud.cc`'s `drawDriverStatus`, not dropped), and 52 modified-upstream files (Group B, full diff-level review). Found and fixed 4 HIGH + 3 MEDIUM + 3 LOW real bugs: `settings.cc` inserting a Bluetooth panel shifted the Toggles panel index, breaking two hardcoded `openSettings(2, ...)` callers (fixed by resolving the Toggles index by name instead of trusting the caller); `widgets/controls.h`'s `ParamSpinBoxControl`/`ParamDoubleSpinBoxControl::refresh()` silently zeroed any unset param's spin box every time its settings panel was shown (fixed by storing and reusing the constructor's default); `sidebar.cc`'s BLE status indicator only checked classic SPP pairing, never the BLE GATT param NavPilot's primary transport actually sets (fixed); four new onroad PIP/pairing overlays in `onroad_home.cc` were missing `Qt::WA_TransparentForMouseEvents`, silently blocking clicks to `ExperimentalButton`/`bev_widget` underneath (fixed); plus `langToEop` missing 3 supported languages, `SetupWidget::openSettings` never emitted, `Networking::setPrimeType()` left as dead code after its wiring was removed, a stale/incorrect comment hiding a working bookmark-button publish (restored), `HttpRequest::active()` permanently `false`, and a dead `handleReply()` declaration (all fixed). Two findings deliberately deferred (unthrottled 20 Hz `Params().get()` reads in `sidebar.cc`/`onroad_home.cc`, and `FirehosePanel` dead-code removal) — both perf/cleanup-only, no correctness impact, and not verifiable without a build environment. Full per-file writeup in `docs/upstream-audit/UI_QT_AUDIT.md`. As with every other pass in this fork's UI work, none of these fixes have been compiled — no `scons`/Qt toolchain available in this environment. | `selfdrive/ui/qt/offroad/settings.cc`, `selfdrive/ui/qt/widgets/controls.h`, `selfdrive/ui/qt/sidebar.cc`, `selfdrive/ui/qt/onroad/onroad_home.cc`, `selfdrive/ui/qt/widgets/prime.cc`, `selfdrive/ui/qt/widgets/prime.h`, `selfdrive/ui/qt/network/networking.cc`, `selfdrive/ui/qt/api.cc`, `selfdrive/ui/qt/api.h`, `selfdrive/ui/qt/prime_state.h`, `docs/upstream-audit/UI_QT_AUDIT.md` (new) |
| 2026-09-01 | **Completed the two `selfdrive/ui/qt` audit findings deferred above.** `sidebar.cc`/`sidebar.h`: added a `ble_watch` `ParamWatcher` (same `QFileSystemWatcher`-based pattern already used by `eop_panel.cc`'s `fs_watch`) covering `BluetoothPairingPin`/`BluetoothPairingActive`/`EOPSPPPairedDevice`/`EOPNavPilotPaired`; `updateState()` now reads four cached member variables refreshed only on actual param-file change, instead of three synchronous `Params().get()` disk reads every 20 Hz UI tick. `onroad_home.cc`/`onroad_home.h`: identical fix for `updatePairingOverlay()` via a new `pairing_watch_` member, replacing its own two per-frame `Params().get()` calls. Deleted the now-fully-dead `FirehosePanel`: removed `selfdrive/ui/qt/offroad/firehose.cc`/`.h` outright, its entry in `selfdrive/ui/SConscript`, and the unused `class FirehosePanel;` forward declaration in `settings.h` (its only remaining reference). Left `widgets/wifi.cc`'s `openSettings(1, "FirehosePanel")` untouched -- it lives inside `WiFiPromptWidget`, which nothing in this fork instantiates, so it was already unreachable dead code before and after. `docs/upstream-audit/UI_QT_AUDIT.md` updated to close out both findings. Not compiled -- no `scons`/Qt toolchain available in this environment. | `selfdrive/ui/qt/sidebar.cc`, `selfdrive/ui/qt/sidebar.h`, `selfdrive/ui/qt/onroad/onroad_home.cc`, `selfdrive/ui/qt/onroad/onroad_home.h`, `selfdrive/ui/qt/offroad/firehose.cc` (deleted), `selfdrive/ui/qt/offroad/firehose.h` (deleted), `selfdrive/ui/SConscript`, `selfdrive/ui/qt/offroad/settings.h`, `docs/upstream-audit/UI_QT_AUDIT.md` |
