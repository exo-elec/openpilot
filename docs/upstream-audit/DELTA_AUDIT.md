...
- **"Convoy/Follow Friend" feature gap identified, not built.** NavPilot's Premium-tier convoy feature (follow a friend's live location as a moving destination, continuously re-routed) has no device-side support yet — `navd.py` doesn't handle a continuously-updating destination, and `bluetoothd/protocol.py` (NCP codes 0x20–0x2F) doesn't implement the relay command NavPilot's docs describe. Scoped as a follow-up task, not started.
