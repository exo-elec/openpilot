...
> **EOP Notice:** EOP is **Tesla-only** via `system/socketd/vehicle/`. The upstream multi-brand car porting architecture (`selfdrive/car/`, `opendbc`, `panda`) has been replaced. This document is preserved from upstream for reference only.
