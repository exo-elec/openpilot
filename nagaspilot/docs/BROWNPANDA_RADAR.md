...
NGP10 is a privileged two-channel consumer of BrownPanda's converted BYD
radar stream. For Tesla Model 3/Y normalization, NGP10 enables radar and reads
the Continental ARS4-B-compatible stream from logical party bus 0. Stock
...
BrownPanda converts ten 64-byte BYD MVS4 CAN-FD slots into classical 8-byte Tesla
...
NGP10's OpenDBC Tesla `RadarInterface` uses the official signal layout while
applying the BrownPanda allocation and safety lifecycle. Tesla fingerprinting enables
it only when both `0x401` and `0x45F` are present as eight-byte frames on party
bus 0. The parser accepts only bus-0 frames, requires
...
BYD `ALEAD` and `LAT_RELATED` remain unproven and reserved. BrownPanda puts neutral
values on the Tesla wire, and NGP10 publishes those optional fields as NaN
