...
| Maximum command delta | 3-step, tapering with speed (see below) | Superseded: this table previously carried the reference's flat `3 deg/20 ms`. `opendbc/safety/modes/byd.h`'s `BYD_STEERING_LIMITS` and `opendbc/car/byd/values.py`'s `CarControllerParams.ANGLE_LIMITS` are now 3-step at NagasPilot's `CRAWL`(0)/`CITY_SPEED_MPS`(12)/`HIGHWAY_SPEED_MPS`(24) m/s breakpoints: `4`/`2`/`.5` deg-per-50Hz-cycle winding up, `4`/`3`/`1.5` unwinding. This is a provisional design (shaped after `psa.h`'s taper, not a route-tested BYD value) at the user's explicit request for CITY/HIGHWAY-differentiated limits — see `MIGRATION_PLAN.md` task 3 for the full rationale and why unwind is always faster than windup. |
