...
| Maximum command delta | Continuous ISO accel/jerk vehicle-model limit, not a step table | Superseded twice: this table originally carried the reference's flat `3 deg/20 ms`, then a provisional 3-step CRAWL/CITY/HIGHWAY taper. Both `opendbc/safety/modes/byd.h` (`steer_angle_cmd_checks_vm`) and `opendbc/car/byd/carcontroller.py` (`apply_steer_angle_limits_vm`) now derive the rate/angle cap continuously from `MAX_LATERAL_ACCEL`/`MAX_LATERAL_JERK` and BYD's own `steer_ratio`/`wheelbase`/slip factor, matching `opendbc/safety/lateral.h`'s ISO 11270-grounded formula. `CarControllerParams.ANGLE_LIMITS`'s `0/2/6/12/24` breakpoint lists still exist (documenting NagasPilot's CRAWL/WALK/CITY/URBAN/HIGHWAY policy ranges from `SPEED_ZONE_POLICY.md`) but are no longer read by `apply_steer_angle_limits_vm` - they're informational only. The BYD-specific slip factor is still a provisional design (not a route-tested capture) - see `MIGRATION_PLAN.md` task 3. |
