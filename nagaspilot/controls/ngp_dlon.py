"""NGP Dynamic Longitudinal Profile (DLON).
...
This is the comma 3 migration of EOP10's DLON behavior. It retains the mode
selection and force-stop policy while using NGP-owned names and dependencies.
"""
...
class NGPDriveMode(Enum):
  """Internal drive mode state."""
  ACC = "acc"
  E2E = "blended"


class NGPModeTransitionManager:
  """Manages smooth transitions between driving modes with hysteresis."""

  def __init__(self):
    self.current_mode = NGPDriveMode.ACC
    self.mode_confidence = {NGPDriveMode.ACC: 1.0, NGPDriveMode.E2E: 0.0}
    self.transition_timeout = 0
    self.mode_duration = 0
    self.emergency_override = False
    self.min_mode_duration = 10  # frames (~0.5s at 20Hz)

  def request_mode(self, mode: NGPDriveMode, confidence: float = 1.0, emergency: bool = False):
  ...
  def get_mode(self) -> NGPDriveMode:
...
class NGPDLON:
  ...
  def __init__(self):
    ...
    # Mode manager
    self.mode_manager = NGPModeTransitionManager()
  ...
  def update(self, sm, mpc_crash_cnt: int = 0) -> dict:
    ...
    if self.mode == NGPDLONMode.CHILL:
      use_e2e = False
    elif self.mode == NGPDLONMode.EXPERIMENTAL:
      use_e2e = True
    else:  # AUTO mode
    ...
    # Update mode manager
    requested_mode = NGPDriveMode.E2E if self._active else NGPDriveMode.ACC
    ...
    is_e2e = final_mode == NGPDriveMode.E2E
