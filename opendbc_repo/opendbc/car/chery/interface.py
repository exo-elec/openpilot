"""Chery/Omoda/iCaur/JAECOO interface: lateral control active, no longitudinal.

Ported from kommuai/opendbc (MIT, commit f8dddb3ba5) 2026-08-12, built active
from the start (no dashcamOnly stage) per explicit request. Unlike BYD Atto 3,
this port had no pre-existing C-side steering backstop to audit against -
opendbc/safety/modes/chery.h's steer_angle_cmd_checks call (mirroring
opendbc/car/chery/values.py's CarControllerParams exactly) is new here, not
carried over from the source. CHERY_JAECOO_J7_PHEV is upstream-marked "under
validation"; CHERY_TIGGO_8_PRO shares an identical CAN fingerprint with it
(see opendbc/car/chery/fingerprints.py) and isn't uniquely identifiable yet.
"""
...
class CarInterface(CarInterfaceBase):
  ...
  @staticmethod
  def _get_params(ret, candidate, fingerprint, car_fw, alpha_long, is_release, dp_params, docs):
    del fingerprint, car_fw, alpha_long, is_release, dp_params, docs
    ret.brand = "chery"
