...
from openpilot.selfdrive.ui.components.dialogs import (
  alert_dialog,
  confirm_dialog,
  selection_dialog,
)
from openpilot.selfdrive.ui.components.list_controls import (
  ButtonControl,
  LabelControl,
  ListWidget,
)
from openpilot.selfdrive.ui.qt import QtWidgets, Signal
...
class DevicePanel(ListWidget):
  ...
  def __init__(self, store: ParamStore | None = None, languages=None, parent=None):
    ...
    self.dongle = LabelControl(self.tr("Dongle ID"), self._text_param("DongleId", "N/A"))
    self.add_row(self.dongle)
    self.serial = LabelControl(self.tr("Serial"), self._text_param("HardwareSerial", "N/A"))
    self.add_row(self.serial)

    self.reset_calib = ButtonControl(self.tr("Reset Calibration"), self.tr("RESET"))
  ...
  def _reset_calibration(self) -> None:
    if not self._guarded(self.tr("Are you sure you want to reset calibration?"),
  ...
  def _change_language(self) -> None:
    ...
    chosen = selection_dialog(self.tr("Select a language"), names, current, self)
    if not chosen:
      return
  ...
  def _reboot(self) -> None:
    if self._guarded(self.tr("Are you sure you want to reboot?"), self.tr("Reboot"),
                     self.tr("Disengage to Reboot")):
      self._store.put_bool("DoReboot", True)

  def _power_off(self) -> None:
    if self._guarded(self.tr("Are you sure you want to power off?"), self.tr("Power Off"),
                     self.tr("Disengage to Power Off")):
...
def supported_languages() -> dict[str, str]:
  ...
  path = Path(__file__).resolve().parents[2] / "translations" / "languages.json"
  try:
