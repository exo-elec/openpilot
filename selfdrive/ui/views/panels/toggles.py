...
from openpilot.selfdrive.ui.qt import Signal
...
class TogglesPanel(ListWidget):
  ...
  def __init__(self, store: ParamStore | None = None, parent=None):
    ...
    for param, title, desc, needs_restart in TOGGLES:
      ...
      # tr() on a variable still resolves at runtime -- Qt looks up
      # (context, source) in the catalogue. Only lupdate's static extraction
      # needs literals, and these strings are already in the .ts files.
      row = ToggleControl(param, self.tr(title), self.tr(full_desc) if full_desc else "",
                          self._store, confirm=param == "ExperimentalMode")
      ...
      if param == "DisengageOnAccelerator":
        self.personality = ButtonParamControl(
          "LongitudinalPersonality", self.tr("Driving Personality"),
          self.tr("How closely openpilot follows lead cars. Standard recommended."),
          [self.tr(p) for p in PERSONALITIES], self._store)
...
class DeveloperPanel(ListWidget):
  ...
  def __init__(self, store: ParamStore | None = None, parent=None):
    ...
    self.adb = ToggleControl(
      "AdbEnabled", self.tr("Enable ADB"),
      self.tr("USB/network debug access. See docs.exo-electronics.com for details."),
      self._store)
    self.add_row(self.adb)

    self.ssh = ToggleControl("SshEnabled", self.tr("Enable SSH"), "", self._store)
    self.add_row(self.ssh)

    self.joystick = ToggleControl("JoystickDebugMode",
                                  self.tr("Joystick Debug Mode"), "", self._store)
    self.add_row(self.joystick)

    self.long_maneuver = ToggleControl("LongitudinalManeuverMode",
                                       self.tr("Longitudinal Maneuver Mode"), "",
                                       self._store)
    ...
    self.alpha_long = ToggleControl(
      ...
      self.tr("WARNING: Disables stock AEB. Switches from car's ACC to openpilot longitudinal (alpha)."),
