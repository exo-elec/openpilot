...
class SoftwarePanel(ListWidget):
  def __init__(self, store: ParamStore | None = None, parent=None):
    ...
    self.version = LabelControl(self.tr("Current Version"), "")
    self.add_row(self.version)

    self.download = ButtonControl(self.tr("Download"), self.tr("CHECK"))
    self.download.clicked.connect(self._on_download)
    self.add_row(self.download)

    self.install = ButtonControl(self.tr("Install Update"), self.tr("INSTALL"))
    self.install.clicked.connect(self._on_install)
    self.add_row(self.install)

    self.target_branch = ButtonControl(self.tr("Target Branch"), self.tr("SELECT"))
    ...
    self.uninstall = ButtonControl(self.tr("Uninstall"), self.tr("UNINSTALL"))
  ...
  def _on_select_branch(self) -> None:
    ...
    chosen = selection_dialog(self.tr("Select a branch"), branches, current, self)
    if not chosen:
      return
  ...
  def _on_uninstall(self) -> None:
    if confirm_dialog(self.tr("Are you sure you want to uninstall?"),
                      self.tr("Uninstall"), self):
