#!/usr/bin/env python3
"""Dynamic bicycle model for vehicles.

Ported from opendbc.car.vehicle_model for Tesla-format centralized protocol.
Based on "The Science of Vehicle Dynamics (2014), M. Guiggiani"
...
class VehicleModel:
  """Dynamic bicycle model for vehicle dynamics calculations."""
  ...
  def __init__(self, CP: car.CarParams):
    ...
    # Estimate rotational inertia if not provided
    # I = m * (aF^2 + aR^2) is a rough approximation
    self.j: float = CP.rotationalInertia if CP.rotationalInertia > 0 else CP.mass * (self.aF**2 + self.aR**2)
    
    # Tire stiffness - estimate if not provided
    # Typical values for passenger cars: cF ~ 150000 N/rad, cR ~ 180000 N/rad
    self.cF_orig: float = CP.tireStiffnessFront if CP.tireStiffnessFront > 0 else 150000.0
    self.cR_orig: float = CP.tireStiffnessRear if CP.tireStiffnessRear > 0 else 180000.0
