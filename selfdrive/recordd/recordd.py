...
class PreImpactBuffer:
    ...
    def get_frames(self, duration_s: float) -> list[tuple[float, np.ndarray]]:
        ...
        with self._lock:
            ...
            frames = []
            for ts, frame in zip(self._frame_times, self._buffer, strict=False):
                if ts >= cutoff_time:
...
class RecordD:
    ...
    def _discover_available_streams(self) -> dict[str, set[VisionStreamType]]:
        ...
        for _stream, server in STREAM_SERVERS.items():
    ...
    def capture_snap(self, reason: str = "manual") -> Path | None:
        ...
        try:
            ...
            # Save
            timestamp_str = datetime.fromtimestamp(time.time()).strftime("%Y%m%d_%H%M%S")  # noqa: TID251
            ...
            while self.running:
                ...
                if self.sm.updated['accelerometer'] and self.sm.updated['gyroscope']:
                    try:
                        ...
                        self._impact_detector.process_imu(accel, gyro_data)
